...
def main(args):
    ...
    ckpt = torch.load('./data/checkpoint_as_init.ckpt', map_location=lambda storage, loc: storage)
    hparams = ckpt['hyper_parameters']
    print(hparams)

    model = SnapshotDepth(hparams=hparams, log_dir=logger.log_dir)

    model.camera.heightmap1d_.data = ckpt['state_dict']['camera.heightmap1d_']
    print("heightmap 1d of the lens:",model.camera.heightmap1d_.data)
